...
function run_uncoded {
    ...
    for i in $(seq 1 3); do
        ssh $CLIENT "sudo ip link set dev eth$i multipath on"
        ssh $SERVER "sudo ip link set dev eth$i multipath on"
    done

    ssh $CLIENT "sudo ip link set dev ncif multipath off"
    ssh $SERVER "sudo ip link set dev ncif multipath off"
...
# 7: Path 3 data distribution
    ...
    for i in $(seq 1 3); do
        ssh $CLIENT "sudo ip link set dev eth$i multipath off"
        ssh $SERVER "sudo ip link set dev eth$i multipath off"
    done

    ssh $CLIENT "sudo ip link set dev ncif multipath on"
    ssh $SERVER "sudo ip link set dev ncif multipath on"
    # tmux new-window -dk -t 6 "ssh $SERVER \"iperf -s\""
...
ssh $CLIENT "sudo ip link set dev eth0 multipath off"
ssh $SERVER "sudo ip link set dev eth0 multipath off"
...
for CYCLE in $(seq 1 10); do
    ...
    for t in 0 1 3; do
        ...
        do
            ...
            IPERF_LOGFILE=iperf_multipath_uncoded_mptcp_topology_$t\_error_$ERROR_RATE.dat
            ...
            for RED in $KODO_REDUNDANCY; do
                ...
                LOG=multipath_kodo_mptcp_distributed_$P1D\_$P2D\_$P3D\_topology_$t\_error_$ERROR_RATE\_redundancy_$RED.dat
