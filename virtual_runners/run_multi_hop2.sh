...
function run_kodo_intermediate {
    if [ $# != 5 ]; then
        echo run_uncoded argument mismatch. Arguments needed: "coding_server_ip iperf_logfile coding_logfile redundancy1 redundancy2"
        exit 1
    fi
    ...
    tmux new-window -dk -t 4 \
        "ssh $CLIENT \"$CODERDIR$CODER ncif $4 /dev/null $CLIENT_IP $RECODER_IP 11111 11111\""
    tmux new-window -dk -t 5 \
        "ssh $RECODER \"$CODERDIR$RCODER $4 $RECODER_IP $CLIENT_IP 11111 11111 $5 $RECODER_IP $SERVER_IP 22222 22222\""
    tmux new-window -dk -t 6 \
        "ssh $SERVER \"$CODERDIR$CODER ncif $5 $3 $SERVER_IP $RECODER_IP 22222 22222\""
...
for i in $(seq 1 10); do
    ...
    for ERROR1 in 0.5 1 5; do
        DELAY1=25
        DELAY1_VAR=$(expr $DELAY1 / 5)

        sudo tc qdisc add dev $DEV11 root netem \
            rate $MAXRATE delay "$DELAY1"ms "$DELAY1_VAR"ms distribution normal loss $ERROR1
        ...
        do
            ...
            for DELAY in 5; do
                ...
                IPERF_LOGFILE=iperf_multihop2_uncoded_delay_$DELAY1\_$DELAY\_error_$ERROR1\_$ERROR_RATE.dat
                run_uncoded $SERVER_IP $LOGDIR$IPERF_LOGFILE
                sleep 1

                for RED in $XOR_REDUNDANCY; do
                    echo Running xor iperf - $DURATION seconds
                    LOG=multihop2_xor_delay_$DELAY1\_$DELAY\_error_$ERROR1\_$ERROR_RATE\_redundancy_$RED.dat
                ...
                for RED in $KODO_REDUNDANCY; do
                    ...
                    LOG=multihop2_kodo_delay_$DELAY1\_$DELAY\_error_$ERROR1\_$ERROR_RATE\_redundancy_$RED.dat
                    IPERF_LOGFILE=iperf_$LOG
                    CODING_LOGFILE=recovery_$LOG
                ...
                for RED1 in $KODO_REDUNDANCY; do
                    for RED2 in $KODO_REDUNDANCY; do
                        echo Running kodo recoding iperf - $DURATION seconds

                        IPERF_LOGFILE=iperf_$LOG
                        CODING_LOGFILE=recovery_$LOG
                        run_kodo_intermediate $CODED_SERVER_IP $LOGDIR$IPERF_LOGFILE $CLOG$CODING_LOGFILE $RED1 $RED2
                        sleep 1
                    done
                done
