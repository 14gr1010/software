...
sudo ovs-vsctl add-port br3 veth12
sudo ovs-vsctl add-port br3 veth22
...
tmux new-window -d -t 3 "kvm \
    -smp 1 \
    -drive file=num3.img,if=virtio \
    -m 1024 \
    -nographic \
    -netdev tap,ifname=tap3,script=no,downscript=no,id=lan0 \
    -device virtio-net-pci,netdev=lan0 \
    -net nic,macaddr=fe:fe:00:0a:03:01 \
    -net tap,ifname=of_tap3,script=no \
    -fsdev local,security_model=passthrough,id=fsdev0,path=$share \
    -device virtio-9p-pci,id=fs0,fsdev=fsdev0,mount_tag=hostshare"
...
sudo ovs-vsctl set-controller br3 tcp:0.0.0.0:6633
