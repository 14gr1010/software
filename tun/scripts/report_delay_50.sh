...
for i in $(seq 1 10); do
    for ERROR_RATE in 0 0.5 1 2 3 4 5 6 8 10; do
        echo "Setting error rate=$ERROR_RATE and delay=${DELAY}ms"
        tc qdisc add dev $DEV root netem loss $ERROR_RATE delay ${DELAY}ms || exit 1
        ssh $remote "tc qdisc add dev $DEV root netem loss $ERROR_RATE delay ${DELAY}ms" || exit 2
        ...
        echo "Removing error rate=$ERROR_RATE and delay=${DELAY}ms"
        tc qdisc del dev $DEV root netem loss $ERROR_RATE delay ${DELAY}ms || exit 3
        ssh $remote "tc qdisc del dev $DEV root netem loss $ERROR_RATE delay ${DELAY}ms" || exit 4
    done
done
